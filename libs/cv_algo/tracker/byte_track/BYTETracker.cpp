...
std::vector<BYTETracker::STrackPtr> BYTETracker::update(const std::vector<Object>& objects)
{
...

    {
        ...
        linearAssignment(dists, static_cast<int>(strack_pool.size()), static_cast<int>(det_stracks.size()), match_thresh_,
...

    {
        ...
        linearAssignment(dists, static_cast<int>(remain_tracked_stracks.size()), static_cast<int>(det_low_stracks.size()), 0.5f,
                         matches_idx, unmatch_track_idx, unmatch_detection_idx);
...

    {
        ...
        linearAssignment(dists, static_cast<int>(non_active_stracks.size()), static_cast<int>(remain_det_stracks.size()), 0.7f,
                                                              ...
                                                              const std::vector<STrackPtr> &b_tlist) const
{
    ...
    {
        int tid = static_cast<int>(b_tlist[i]->getTrackId());
                                         ...
                                         std::vector<STrackPtr> &b_res) const
{
    ...
    {
        int timep = static_cast<int>(a_stracks[a_idx]->getFrameId() - a_stracks[a_idx]->getStartFrameId());
        int timeq = static_cast<int>(b_stracks[b_idx]->getFrameId() - b_stracks[b_idx]->getStartFrameId());
        if (timep > timeq)
        {
                                   ...
                                   std::vector<int> &b_unmatched) const
{
    ...
    for (size_t i = 0; i < colsol.size(); ++i)
    {
        if (colsol[i] < 0)
        {
            b_unmatched.push_back(static_cast<int>(i));
        }
    }
}
