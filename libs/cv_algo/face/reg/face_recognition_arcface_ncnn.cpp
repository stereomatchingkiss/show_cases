...
{
    impl(std::string const &param, std::string const &bin, int target_size, bool swap_rgb) :
    ...
    std::vector<float> predict(cv::Mat const &mat)
    {
        ...
        ncnn::Mat input = swap_rgb_ ? ncnn::Mat::from_pixels_resize(mat.data, ncnn::Mat::PIXEL_BGR2RGB,
                                                                    mat.cols, mat.rows,
                                                                    target_size_, target_size_) :
                              ncnn::Mat::from_pixels_resize(mat.data, ncnn::Mat::PIXEL_RGB,
                                                            mat.cols, mat.rows,
                                                            target_size_, target_size_);
...
face_recognition_arcface_ncnn::face_recognition_arcface_ncnn(std::string const &param, std::string const &bin, int target_size, bool swap_rgb) :
    face_recognition_base(),
    impl_{std::make_unique<impl>(param, bin, target_size, swap_rgb)}
{    
}
...
std::vector<float> face_recognition_arcface_ncnn::predict(const cv::Mat &bgr)
{
