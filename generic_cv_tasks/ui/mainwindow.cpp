...
void MainWindow::on_pushButtonNext_clicked()
{
    if(ui->stackedWidget->currentWidget() == widget_source_selection_){
        ui->labelTitle->setText(tr(""));
        ui->stackedWidget->setCurrentWidget(widget_stream_player_);
        ui->pushButtonNext->setEnabled(false);
        ui->pushButtonPrev->setEnabled(true);
        frame_capture_params params;
        params.max_fps_ = 30;
        params.url_ = widget_source_selection_->get_url().toStdString();
        //params.url_ = "D:/programming/videos/Traffic count, monitoring with computer vision. 4K, UHD, HD..mp4";
        sfwmw_ = std::make_unique<single_frame_with_multi_worker>(params);
        auto process_controller = std::make_shared<frame_process_controller>(new nanodet_worker(0.25));
